...
onMounted(async () => {
  if (isEdit.value) {
    loading.value = true
    try {
      const { data } = await api.get(`/api/sliders/${route.params.id}`)
      form.value = {
        title: data.title ?? '',
      ...
      error.value = 'Slider yüklenemedi.'
    } finally {
      loading.value = false
    }
  }
...
async function submit() {
  ...
  try {
    const payload = {
      ...
      shortText: form.value.shortText || null,
      imageUrl: form.value.imageUrl || null,
      sortOrder: Number(form.value.sortOrder) || 0,
      isActive: !!form.value.isActive
    }
    if (isEdit.value) {
      await api.put(`/api/sliders/${route.params.id}`, payload)
      router.push('/cms/sliders')
    } else {
...
<template>
  <div>
    <PageHeader :title="isEdit ? 'Slider Düzenle' : 'Yeni Slider'" :subtitle="isEdit ? 'Slider bilgilerini güncelleyin' : 'Yeni slider ekleyin'" />
    <FormCard title="Slider Bilgileri" :loading="loading">
      ...
      <form v-else @submit.prevent="submit" class="form-grid">
        ...
        <div class="field full">
          <label for="shortText">Kısa Metin</label>
          <input id="shortText" v-model="form.shortText" type="text" maxlength="500" />
        </div>
        <div class="field full">
